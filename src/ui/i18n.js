// Interface language: English and Simplified Chinese.
// t('key', { name: value }) looks a string up in the current language and fills {name} slots.
// Static markup is translated through data-i18n (text), data-i18n-html (trusted markup) and
// data-i18n-attr ("placeholder:key; aria-label:key") attributes.

const DICT = {
  en: {
    'app.name': 'Lumencraft',
    'lang.en': 'English',
    'lang.zh': '中文',

    // title
    'title.eyebrow': 'Voxel sandbox · WebGL 2',
    'title.tagline': 'An endless blocky world with a physically based sky, soft shadows, volumetric clouds and water that reflects it all.',
    'title.play': 'Play',
    'title.continue': 'Continue',
    'title.new': 'New world',
    'title.settings': 'Settings',
    'title.help': 'Controls',
    'title.device': 'Device check',
    'title.quickstart': 'Quick start',
    'title.meta': 'Seed {seed} · {state}',
    'title.savedWorld': 'saved world',
    'title.newWorld': 'new world',
    'title.language': 'Language',

    // pause
    'pause.title': 'Paused',
    'pause.resume': 'Resume',
    'pause.toTitle': 'Save and go to title',

    // settings
    'settings.title': 'Settings',
    'settings.hint': 'Changes apply immediately.',
    'settings.done': 'Done',
    'tab.general': 'General',
    'tab.graphics': 'Graphics',
    'tab.world': 'World',
    'tab.controls': 'Controls',
    'tab.sound': 'Sound',
    'set.language': 'Language',
    'set.preset': 'Quality preset',
    'set.preset.desc': 'Lite is made for phones and projectors. Ultra doubles shadow resolution and renders at full display density.',
    'preset.lite': 'Lite',
    'preset.low': 'Low',
    'preset.medium': 'Medium',
    'preset.high': 'High',
    'preset.ultra': 'Ultra',
    'preset.custom': 'Custom',
    'set.renderDistance': 'Render distance',
    'set.texturePack': 'Textures',
    'set.texturePack.desc': 'Pixel keeps the classic 16×16 look. HD uses detailed 64×64 materials with depth.',
    'pack.pixel': 'Pixel',
    'pack.hd': 'HD',
    'set.grass3d': '3D grass',
    'set.grass3d.desc': 'Real blades on grass blocks near you that sway in the wind and part as you walk.',
    'set.fancyLeaves': 'Fluffy leaves',
    'set.fancyLeaves.desc': 'Extra leaf cards that break up the cube outline of tree tops.',
    'set.pom': 'Parallax depth',
    'set.pom.desc': 'HD textures get real relief and self-shadowing up close.',
    'set.renderScale': 'Resolution scale',
    'set.dynamicRes': 'Dynamic resolution',
    'set.dynamicRes.desc': 'Lowers the resolution while the frame rate drops, and raises it again when there is headroom.',
    'set.targetFps': 'Target frame rate',
    'set.shadows': 'Soft shadows',
    'set.shadows.desc': 'Sun and moon shadows with contact hardening.',
    'set.clouds': 'Volumetric clouds',
    'set.clouds.desc': 'Raymarched cumulus that also shade the ground.',
    'set.volumetric': 'God rays',
    'set.volumetric.desc': 'Light shafts through leaves, fog and water.',
    'set.ssr': 'Water reflections',
    'set.ssr.desc': 'Screen-space reflections of terrain on water.',
    'set.ssao': 'Ambient occlusion',
    'set.ssao.desc': 'Extra contact shading between plants and blocks.',
    'set.bloom': 'Bloom',
    'set.bloom.desc': 'Glow around the sun, torches and bright sky.',
    'set.taa': 'Temporal anti-aliasing',
    'set.taa.desc': 'Smooth edges and stable shimmer; off uses FXAA.',
    'set.weather': 'Weather',
    'set.weather.desc': 'Changing brings rain now and then, with the odd thunderstorm. Deserts stay dry; cold biomes get snow.',
    'weather.auto': 'Changing',
    'weather.clear': 'Clear',
    'weather.rain': 'Rain',
    'weather.storm': 'Storm',
    'set.timeOfDay': 'Time of day',
    'set.dayLength': 'Day length',
    'set.cloudCoverage': 'Cloud cover',
    'set.brightness': 'Brightness',
    'set.fov': 'Field of view',
    'set.sensitivity': 'Mouse sensitivity',
    'set.invertY': 'Invert vertical look',
    'set.viewBobbing': 'View bobbing',
    'set.autoJump': 'Auto-jump',
    'set.autoJump.desc': 'Step up single blocks while walking into them.',
    'set.padSensitivity': 'Controller look speed',
    'set.padInvertY': 'Controller: invert vertical look',
    'set.padVibration': 'Controller vibration',
    'set.touchSize': 'Touch button size',
    'set.touchOpacity': 'Touch button opacity',
    'set.touchSensitivity': 'Touch look speed',
    'set.touchHaptics': 'Vibrate on touch actions',
    'set.touchHaptics.desc': 'Short buzzes when you break or place blocks (Android).',
    'set.volume': 'Volume',
    'set.ambience': 'Ambient sounds',
    'set.ambience.desc': 'Wind, birds by day, crickets at night.',
    'unit.chunks': '{n} chunks',
    'unit.minutes': '{n} min',
    'unit.fps': '{n} fps',

    // help
    'help.title': 'Controls',
    'help.back': 'Back',
    'help.keyboard': 'Keyboard and mouse',
    'help.gamepad': 'Controller',
    'help.touch': 'Touch screen',
    'help.remote': 'In menus, a TV remote works too: arrows to move, OK to choose, Back to return.',
    'key.leftClick': 'Left click',
    'key.rightClick': 'Right click',
    'key.middleClick': 'Middle click',
    'key.mouse': 'Mouse',
    'key.wheel': 'wheel',
    'key.or': 'or',
    'act.walk': 'Walk',
    'act.look': 'Look around',
    'act.jump': 'Jump, swim up, fly up',
    'act.toggleFly': 'Toggle flying',
    'act.sneak': 'Sneak, fly down',
    'act.sprint': 'Sprint',
    'act.break': 'Break blocks, attack',
    'act.place': 'Place blocks, eat, draw the bow',
    'act.pick': "Pick the block you're looking at",
    'act.hotbar': 'Choose hotbar slot',
    'act.inventory': 'Inventory and crafting',
    'act.drop': 'Drop the held item',
    'act.fastTime': 'Hold to fast-forward the day',
    'act.debug': 'Debug info',
    'act.hideHud': 'Hide the interface',
    'act.pause': 'Pause',
    'act.move': 'Move',
    'act.moveSprint': 'Move (click to sprint)',
    'act.jumpFly': 'Jump · twice to fly',
    'act.breakBlock': 'Break / attack',
    'act.placeBlock': 'Place / use',
    'act.allBlocks': 'Inventory',
    'act.holdTime': 'Hold to speed up time',
    'pad.leftStick': 'Left stick',
    'pad.rightStick': 'Right stick',
    'pad.dpad': 'D-pad',
    'pad.menu': 'Menu',
    'pad.view': 'View',
    'act.lookPick': 'Look (click to pick block)',
    'act.hotbarPad': 'Previous / next slot',
    'act.menuNav': 'In menus: D-pad or stick to move, A to choose, B to go back, LB / RB to switch tabs',
    'touch.stick': 'Left stick',
    'touch.stickDesc': 'Move; push to the edge to sprint',
    'touch.drag': 'Drag',
    'touch.dragDesc': 'Look around',
    'touch.tap': 'Tap',
    'touch.tapDesc': 'Place a block, eat, shoot',
    'touch.hold': 'Hold',
    'touch.holdDesc': 'Break a block or attack',
    'touch.hotbar': 'Hotbar',
    'touch.hotbarDesc': 'Tap a slot to select it',
    'touch.buttons': 'Buttons',
    'touch.buttonsDesc': 'Jump, sneak, fly (creative), attack, use, inventory, pause',
    'touch.jump': 'Jump',
    'touch.sneak': 'Sneak or fly down',
    'touch.fly': 'Toggle flying',
    'touch.flyLabel': 'FLY',
    'touch.inventory': 'Inventory',
    'touch.pause': 'Pause',
    'touch.break': 'Break / attack',
    'touch.place': 'Place / use',
    'touch.fullscreen': 'Full screen',
    'touch.rotate': 'Turn your phone sideways for the best view',

    // new world
    'new.title': 'New world',
    'new.hint': 'This replaces the world you have now, including everything you built.',
    'new.seed': 'Seed (leave empty for a random one)',
    'new.placeholder': 'e.g. lighthouse',
    'new.cancel': 'Cancel',
    'new.create': 'Create world',

    // inventory
    'inv.title': 'Blocks and items',
    'inv.hint': 'Click a block or item to pick up a stack (right click: one), then click or drag it into any slot. Shift-click puts a stack straight into your bag. Drop it on the bin to throw it away. E closes.',
    'inv.survivalTitle': 'Inventory',
    'inv.survivalHint': 'Click a stack to pick it up (right click: half), click or drag it to another slot to put it there. Shift-click moves it between bag and hotbar, and armour onto your body. Craft from what you carry. E closes.',
    'inv.bag': 'Bag',
    'inv.crafting': 'Crafting',
    'inv.planks': 'any planks',
    'inv.noRecipes': 'Nothing to craft yet. Chop a tree to get started.',
    'inv.more': 'Recipes you can not make yet',
    'inv.full': 'Inventory full',
    'inv.crafted': 'Crafted {item}',
    'inv.armor': 'Armour',
    'inv.armorPts': '{n} armour',
    'inv.armorSlot.0': 'Helmet',
    'inv.armorSlot.1': 'Chestplate',
    'inv.armorSlot.2': 'Leggings',
    'inv.armorSlot.3': 'Boots',
    'inv.search': 'Search',
    'inv.trash': 'Bin',
    'inv.tab.all': 'All',
    'inv.tab.building': 'Building',
    'inv.tab.nature': 'Nature',
    'inv.tab.tools': 'Tools',
    'inv.tab.combat': 'Combat',
    'inv.tab.food': 'Food',
    'inv.tab.misc': 'Materials',
    'hud.armor': 'Armour {n}',
    'bed.spawnSet': 'Respawn point set',
    'bed.day': 'You can only sleep at night or during thunderstorms',
    'bed.monsters': 'You may not rest now: there are monsters nearby',
    'bed.morning': 'Good morning',
    'bed.missing': 'Your bed is gone, so you wake up at the world spawn',
    'bed.sleeping': 'Sleeping…',
    'bed.sleepers': 'Sleeping… {n} of {m} players are in bed',
    'bed.leave': 'Leave bed',
    'dim.enter.overworld': 'The Overworld',
    'dim.enter.nether': 'The Nether',
    'dim.enter.end': 'The End',
    'dim.endPortalOpen': 'The end portal opens',
    'dim.victory': 'You beat the Ender Dragon. The End has been conquered.',
    'boss.dragon': 'Ender Dragon',
    'boss.slain': 'The Ender Dragon has been slain!',
    'mp.dragon': '{name} slew the Ender Dragon!',
    'death.zombified_piglin': 'Cut down by a zombified piglin.',
    'death.blaze': 'Burned by a blaze.',
    'death.ghast': 'Blown up by a ghast.',
    'death.enderman': 'Torn apart by an enderman.',
    'death.ender_dragon': 'Slain by the Ender Dragon.',
    'death.magma': 'Discovered the floor was lava.',
    'death.pearl': 'Took one ender pearl too many.',
    'help.adventure': 'Adventures',
    'adv.bed': 'Right click a bed at night to sleep until morning (in multiplayer, everyone in the overworld has to be in bed). It also becomes where you respawn.',
    'adv.farm': 'Right click grass or dirt with a hoe to till it, then plant wheat seeds on it. Wheat grows while it is light.',
    'adv.nether': 'Build an obsidian frame (4 wide, 5 tall) and light it with flint and steel. Stand in the portal to go to the Nether.',
    'adv.end': 'Blaze rods (from blazes in nether fortresses) and ender pearls (from endermen) make eyes of ender. Throw one and follow it to a stronghold, fill the 12 portal frames, and jump in to fight the dragon.',
    'adv.armor': 'Shift-click armour in the inventory, or drag it onto the armour slots, to wear it.',

    // loading & errors
    'load.title': 'Shaping terrain',
    'load.sub': 'Generating chunks…',
    'load.chunks': '{ready} / {total} chunks near you',
    'err.title': "Can't start the renderer",
    'err.hint': 'Lumencraft needs WebGL 2 with floating-point render targets. Recent Chrome, Edge, Firefox and Safari support it; check that hardware acceleration is turned on.',
    'err.contextLost': 'The graphics context was lost (the GPU was reset or ran out of memory). Your world was saved; reload the page to continue.',
    'err.noWebgl2': 'This browser does not support WebGL 2.',
    'err.noFloat': 'This GPU cannot render to floating-point textures.',

    // HUD & toasts
    'hud.lockHint': 'Click to look around',
    'hud.dragHint': 'Drag with the mouse to look around',
    'hud.gameView': 'Game view',
    'hud.hotbar': 'Hotbar',
    'toast.newWorld': 'New world created',
    'toast.time': 'Time {time}',
    'toast.autoQuality': 'Running at {fps} fps, switched graphics to {preset}. Change it in Settings.',
    'toast.error': 'Something went wrong: {msg}',
    'toast.padConnected': 'Controller connected: {name}',
    'toast.padDisconnected': 'Controller disconnected',
    'toast.creative': 'Creative mode',
    'toast.survival': 'Survival mode',
    'toast.toolBroke': 'Your {item} broke',
    'toast.dusk': 'Night is falling. Monsters come out in the dark.',
    'hud.health': 'Health {n} / {max}',
    'hud.air': 'Air',

    // game modes, difficulty, death
    'new.mode': 'Game mode',
    'new.difficulty': 'Difficulty',
    'mode.survival': 'Survival',
    'mode.creative': 'Creative',
    'mode.survival.desc': 'Gather, craft and make it through the night. Monsters come out in the dark.',
    'mode.creative.desc': 'Every block and item, flying, and nothing can hurt you.',
    'diff.peaceful': 'Peaceful',
    'diff.easy': 'Easy',
    'diff.normal': 'Normal',
    'diff.hard': 'Hard',
    'set.gameMode': 'Game mode',
    'set.gameMode.desc': 'Switch this world between survival and creative at any time.',
    'set.difficulty': 'Difficulty',
    'set.difficulty.desc': 'Peaceful has no monsters. Harder settings bring more of them, and they hit harder.',
    'death.title': 'You died',
    'death.note': 'Everything you carried is lying where you fell. Go back for it before it disappears.',
    'death.respawn': 'Respawn',
    'death.zombie': 'A zombie got you.',
    'death.skeleton': 'Shot by a skeleton.',
    'death.arrow': 'Shot by an arrow.',
    'death.spider': 'Bitten by a spider.',
    'death.creeper': 'Blown up by a creeper.',
    'death.explosion': 'Caught in an explosion.',
    'death.fall': 'Fell from a high place.',
    'death.drown': 'Drowned.',
    'death.lava': 'Tried to swim in lava.',
    'death.fire': 'Burned to death.',
    'death.cactus': 'Pricked to death by a cactus.',
    'death.void': 'Fell out of the world.',
    'death.other': 'Your adventure ended here.',

// multiplayer
    'title.multi': 'Multiplayer',
    'mp.title': 'Play with friends',
    'mp.hint': 'Enter the address of a Lumencraft server. Everyone on it shares one world and can talk by voice.',
    'mp.hintHere': 'This page comes from the server "{server}": leave the address empty to join it.',
    'mp.address': 'Server address',
    'mp.addressPlaceholder': 'e.g. game.example.com',
    'mp.here': 'Empty: {server}',
    'mp.name': 'Your name',
    'mp.password': 'Password (only if the server has one)',
    'mp.join': 'Join',
    'mp.connecting': 'Connecting…',
    'mp.err.password': 'Wrong password.',
    'mp.err.full': 'The server is full.',
    'mp.err.nameTaken': 'Someone with that name is already playing.',
    'mp.err.badName': 'Choose a name first.',
    'mp.err.version': 'The server runs a different version of the game. Reload the page.',
    'mp.err.unreachable': 'Could not reach the server. Check the address and that the server is running.',
    'mp.err.timeout': 'The server did not answer in time.',
    'mp.err.address': 'Enter a server address.',
    'mp.err.sandbox': 'Multiplayer cannot run inside this preview page. Open the game from your own server.',
    'mp.welcome': 'Joined {server}. Press Enter to chat, V for the microphone.',
    'mp.joined': '{name} joined the game',
    'mp.left': '{name} left the game',
    'mp.died': '{name} died',
    'mp.lost': 'Lost the connection to the server',
    'mp.leave': 'Leave the server',
    'mp.players': '{n} online',
    'mp.you': 'you',
    'mp.chat': 'Chat',
    'mp.chatPlaceholder': 'Say something… Enter sends, Esc closes',
    'voice.micOff': 'Turn on the microphone',
    'voice.micOn': 'Microphone on',
    'voice.mute': 'Mute the microphone',
    'voice.unmute': 'Unmute the microphone',
    'voice.muted': 'Microphone muted',
    'voice.proximity': 'Nearby',
    'voice.global': 'Everyone',
    'voice.hint': 'Nearby: voices fade with distance and come from where people stand. Everyone: all players at full volume. V (or D-pad up) switches the microphone.',
    'voice.speaking': '{names} talking',
    'voice.state.off': 'No microphone',
    'voice.state.on': 'Microphone on',
    'voice.state.muted': 'Muted',
    'voice.state.speaking': 'Talking',
    'voice.err.insecure': 'The microphone only works when the game is opened from an https:// address.',
    'voice.err.denied': 'Microphone permission was denied. Allow it in the browser settings for this site.',
    'voice.err.unsupported': "This browser can't do voice chat. You can still hear the others.",
    'set.voiceVolume': 'Voice chat volume',
    'set.voiceMode': 'Voice chat',
    'act.chat': 'Chat (multiplayer)',
    'act.mic': 'Microphone on / off (multiplayer)',
    'touch.chat': 'Chat',
    'touch.mic': 'Microphone',

    // device check
    'dev.title': 'Device check',
    'dev.hint': 'Take a screenshot of this page if something does not work, and send it along with your description.',
    'dev.browser': 'Browser',
    'dev.screen': 'Screen',
    'dev.gpu': 'Graphics',
    'dev.webgl2': 'WebGL 2',
    'dev.floatTargets': 'Float render targets',
    'dev.maxTexture': 'Max texture size',
    'dev.cores': 'CPU threads',
    'dev.memory': 'Memory',
    'dev.touch': 'Touch screen',
    'dev.gamepads': 'Controllers',
    'dev.gamepadsNone': 'None found. Connect one and press any button.',
    'dev.fps': 'Frame rate',
    'dev.preset': 'Quality preset',
    'dev.resolution': 'Render resolution',
    'dev.audio': 'Web Audio',
    'dev.storage': 'Saving',
    'dev.yes': 'Yes',
    'dev.no': 'No',
    'dev.recommend': 'Suggested preset: {preset}',
    'dev.back': 'Back',

    // debug overlay
    'dbg.cpu': 'cpu',
    'dbg.facing': 'facing',
    'dbg.dirs': 'north (-Z)|east (+X)|south (+Z)|west (-X)',
    'dbg.biome': 'Biome',
    'dbg.light': 'light sky {sky} block {block}',
    'dbg.time': 'Time',
    'dbg.weather': 'weather',
    'dbg.flying': 'flying',
    'dbg.swimming': 'swimming',
    'dbg.onGround': 'on ground',
    'dbg.airborne': 'airborne',
    'dbg.chunks': 'Chunks {loaded} loaded, {drawn} drawn, {shadow} in shadow pass',
    'dbg.render': 'Render {size}  preset {preset}  shadows {shadows}',
    'dbg.off': 'off',
    'dbg.seed': 'Seed',
    'dbg.entities': 'Entities {n}   {mode} · {diff}',

    biomes: 'Ocean|Beach|Plains|Forest|Birch Forest|Taiga|Snowy Taiga|Desert|Mountains|River',
  },

  zh: {
    'app.name': 'Lumencraft',
    'lang.en': 'English',
    'lang.zh': '中文',

    'title.eyebrow': '方块沙盒 · WebGL 2',
    'title.tagline': '一个无边无际的方块世界：基于物理的天空、柔和阴影、体积云，还有倒映着这一切的水面。',
    'title.play': '开始游戏',
    'title.continue': '继续游戏',
    'title.new': '新建世界',
    'title.settings': '设置',
    'title.help': '操作说明',
    'title.device': '设备检测',
    'title.quickstart': '快速上手',
    'title.meta': '种子 {seed} · {state}',
    'title.savedWorld': '已保存的世界',
    'title.newWorld': '新世界',
    'title.language': '语言',

    'pause.title': '已暂停',
    'pause.resume': '继续',
    'pause.toTitle': '保存并返回标题',

    'settings.title': '设置',
    'settings.hint': '修改会立即生效。',
    'settings.done': '完成',
    'tab.general': '通用',
    'tab.graphics': '画面',
    'tab.world': '世界',
    'tab.controls': '操作',
    'tab.sound': '声音',
    'set.language': '语言',
    'set.preset': '画质预设',
    'set.preset.desc': '"流畅"专为手机和投影仪设计；"极致"使用双倍阴影分辨率，并按屏幕原生像素密度渲染。',
    'preset.lite': '流畅',
    'preset.low': '低',
    'preset.medium': '中',
    'preset.high': '高',
    'preset.ultra': '极致',
    'preset.custom': '自定义',
    'set.renderDistance': '视野距离',
    'set.texturePack': '材质',
    'set.texturePack.desc': '"像素"保持经典的 16×16 风格；"高清"使用有立体感的 64×64 细节材质。',
    'pack.pixel': '像素',
    'pack.hd': '高清',
    'set.grass3d': '立体草地',
    'set.grass3d.desc': '在你周围的草方块上长出真正的草叶，会随风摆动，走过时会被拨开。',
    'set.fancyLeaves': '蓬松树叶',
    'set.fancyLeaves.desc': '额外的叶片让树冠不再是一个个方块。',
    'set.pom': '视差深度',
    'set.pom.desc': '近处的高清材质会有真实的凹凸和自阴影。',
    'set.renderScale': '分辨率比例',
    'set.dynamicRes': '动态分辨率',
    'set.dynamicRes.desc': '帧率下降时自动降低分辨率，性能有余量时再升回来。',
    'set.targetFps': '目标帧率',
    'set.shadows': '柔和阴影',
    'set.shadows.desc': '日光和月光的阴影，离物体越近越清晰。',
    'set.clouds': '体积云',
    'set.clouds.desc': '光线步进渲染的积云，还会在地面投下云影。',
    'set.volumetric': '体积光（丁达尔效应）',
    'set.volumetric.desc': '穿过树叶、雾气和水面的光束。',
    'set.ssr': '水面反射',
    'set.ssr.desc': '在水面上倒映出地形（屏幕空间反射）。',
    'set.ssao': '环境光遮蔽',
    'set.ssao.desc': '让植物与方块的接触处更有阴影层次。',
    'set.bloom': '泛光',
    'set.bloom.desc': '太阳、火把和明亮天空周围的光晕。',
    'set.taa': '时间抗锯齿（TAA）',
    'set.taa.desc': '边缘更平滑、画面不闪烁；关闭后改用 FXAA。',
    'set.weather': '天气',
    'set.weather.desc': '"多变"会不时下雨，偶尔还有雷暴。沙漠不下雨，寒冷地区会下雪。',
    'weather.auto': '多变',
    'weather.clear': '晴天',
    'weather.rain': '下雨',
    'weather.storm': '雷暴',
    'set.timeOfDay': '时间',
    'set.dayLength': '一天时长',
    'set.cloudCoverage': '云量',
    'set.brightness': '亮度',
    'set.fov': '视野角度',
    'set.sensitivity': '鼠标灵敏度',
    'set.invertY': '反转上下视角',
    'set.viewBobbing': '走路时镜头晃动',
    'set.autoJump': '自动跳跃',
    'set.autoJump.desc': '走向一格高的方块时自动跳上去。',
    'set.padSensitivity': '手柄视角速度',
    'set.padInvertY': '手柄：反转上下视角',
    'set.padVibration': '手柄震动',
    'set.touchSize': '触屏按钮大小',
    'set.touchOpacity': '触屏按钮不透明度',
    'set.touchSensitivity': '触屏视角速度',
    'set.touchHaptics': '触屏震动反馈',
    'set.touchHaptics.desc': '挖掘和放置方块时轻微震动（安卓）。',
    'set.volume': '音量',
    'set.ambience': '环境音',
    'set.ambience.desc': '风声，白天的鸟叫，夜晚的虫鸣。',
    'unit.chunks': '{n} 个区块',
    'unit.minutes': '{n} 分钟',
    'unit.fps': '{n} 帧',

    'help.title': '操作说明',
    'help.back': '返回',
    'help.keyboard': '键盘和鼠标',
    'help.gamepad': '手柄',
    'help.touch': '触屏',
    'help.remote': '在菜单里也可以用电视遥控器：方向键移动，确认键选择，返回键后退。',
    'key.leftClick': '鼠标左键',
    'key.rightClick': '鼠标右键',
    'key.middleClick': '鼠标中键',
    'key.mouse': '鼠标',
    'key.wheel': '滚轮',
    'key.or': '或',
    'act.walk': '移动',
    'act.look': '转动视角',
    'act.jump': '跳跃、上浮、向上飞',
    'act.toggleFly': '开关飞行',
    'act.sneak': '潜行、向下飞',
    'act.sprint': '疾跑',
    'act.break': '挖掘方块、攻击',
    'act.place': '放置方块、吃东西、拉弓',
    'act.pick': '选取准星对着的方块',
    'act.hotbar': '选择物品栏格子',
    'act.inventory': '背包和合成',
    'act.drop': '丢出手中的物品',
    'act.fastTime': '按住让时间快进',
    'act.debug': '调试信息',
    'act.hideHud': '隐藏界面',
    'act.pause': '暂停',
    'act.move': '移动',
    'act.moveSprint': '移动（按下摇杆疾跑）',
    'act.jumpFly': '跳跃 · 连按两次飞行',
    'act.breakBlock': '挖掘 / 攻击',
    'act.placeBlock': '放置 / 使用',
    'act.allBlocks': '背包',
    'act.holdTime': '按住加快时间',
    'pad.leftStick': '左摇杆',
    'pad.rightStick': '右摇杆',
    'pad.dpad': '方向键',
    'pad.menu': '菜单键',
    'pad.view': '视图键',
    'act.lookPick': '视角（按下选取方块）',
    'act.hotbarPad': '上一格 / 下一格',
    'act.menuNav': '菜单中：方向键或摇杆移动，A 确认，B 返回，LB / RB 切换标签页',
    'touch.stick': '左侧摇杆',
    'touch.stickDesc': '移动；推到边缘就是疾跑',
    'touch.drag': '滑动',
    'touch.dragDesc': '转动视角',
    'touch.tap': '轻点',
    'touch.tapDesc': '放置方块、吃东西、射箭',
    'touch.hold': '长按',
    'touch.holdDesc': '挖掘方块或攻击',
    'touch.hotbar': '物品栏',
    'touch.hotbarDesc': '点一下格子就能选中',
    'touch.buttons': '按钮',
    'touch.buttonsDesc': '跳跃、潜行、飞行（创造）、攻击、使用、背包、暂停',
    'touch.jump': '跳跃',
    'touch.sneak': '潜行或向下飞',
    'touch.fly': '开关飞行',
    'touch.flyLabel': '飞',
    'touch.inventory': '背包',
    'touch.pause': '暂停',
    'touch.break': '挖掘 / 攻击',
    'touch.place': '放置 / 使用',
    'touch.fullscreen': '全屏',
    'touch.rotate': '横过手机来玩效果更好',

    'new.title': '新建世界',
    'new.hint': '这会替换掉你现在的世界，包括你建造的一切。',
    'new.seed': '种子（留空则随机）',
    'new.placeholder': '例如：灯塔',
    'new.cancel': '取消',
    'new.create': '创建世界',

    'inv.title': '方块和物品',
    'inv.hint': '点一下方块或物品拿起一整组（右键拿一个），再点或拖到任意格子里。Shift+点击直接放进背包。拖到垃圾桶就扔掉。按 E 关闭。',
    'inv.survivalTitle': '背包',
    'inv.survivalHint': '点一下物品拿起来（右键拿一半），再点或拖到别的格子放下。Shift+点击在背包和快捷栏之间移动，盔甲会直接穿上。用身上的材料合成。按 E 关闭。',
    'inv.bag': '背包',
    'inv.crafting': '合成',
    'inv.planks': '任意木板',
    'inv.noRecipes': '现在还合成不了东西。先去砍棵树吧。',
    'inv.more': '材料还不够的配方',
    'inv.full': '背包满了',
    'inv.crafted': '合成了{item}',
    'inv.armor': '盔甲',
    'inv.armorPts': '护甲 {n}',
    'inv.armorSlot.0': '头盔',
    'inv.armorSlot.1': '胸甲',
    'inv.armorSlot.2': '护腿',
    'inv.armorSlot.3': '靴子',
    'inv.search': '搜索',
    'inv.trash': '垃圾桶',
    'inv.tab.all': '全部',
    'inv.tab.building': '建筑',
    'inv.tab.nature': '自然',
    'inv.tab.tools': '工具',
    'inv.tab.combat': '战斗',
    'inv.tab.food': '食物',
    'inv.tab.misc': '材料',
    'hud.armor': '护甲 {n}',
    'bed.spawnSet': '已设置重生点',
    'bed.day': '只能在夜晚或雷雨天睡觉',
    'bed.monsters': '现在还不能休息，附近有怪物',
    'bed.morning': '早上好',
    'bed.missing': '你的床不见了，只能回到世界出生点',
    'bed.sleeping': '正在睡觉…',
    'bed.sleepers': '正在睡觉… {m} 位玩家中有 {n} 位已经上床',
    'bed.leave': '起床',
    'dim.enter.overworld': '主世界',
    'dim.enter.nether': '下界',
    'dim.enter.end': '末地',
    'dim.endPortalOpen': '末地传送门打开了',
    'dim.victory': '你打败了末影龙，征服了末地！',
    'boss.dragon': '末影龙',
    'boss.slain': '末影龙被打败了！',
    'mp.dragon': '{name} 打败了末影龙！',
    'death.zombified_piglin': '被僵尸猪灵砍倒了。',
    'death.blaze': '被烈焰人烧死了。',
    'death.ghast': '被恶魂的火球炸死了。',
    'death.enderman': '被末影人撕碎了。',
    'death.ender_dragon': '被末影龙杀死了。',
    'death.magma': '发现了地板是岩浆。',
    'death.pearl': '末影珍珠扔得太多了。',
    'help.adventure': '冒险指南',
    'adv.bed': '晚上右键点床就能一觉睡到天亮（多人游戏时，主世界里所有人都要上床）。床也会成为你的重生点。',
    'adv.farm': '拿锄头右键点草方块或泥土来耕地，再在耕地上种小麦种子。有光照的时候小麦就会长大。',
    'adv.nether': '用黑曜石搭一个框（宽 4 格、高 5 格），用打火石点燃。站进传送门就能去下界。',
    'adv.end': '烈焰棒（下界要塞里的烈焰人掉落）和末影珍珠（末影人掉落）可以做成末影之眼。扔出去跟着它找到要塞，把 12 个传送门框架都放上末影之眼，跳进去挑战末影龙。',
    'adv.armor': '在背包里 Shift+点击盔甲，或者把它拖到盔甲格上，就能穿上。',

    'load.title': '正在塑造地形',
    'load.sub': '正在生成区块…',
    'load.chunks': '附近区块 {ready} / {total}',
    'err.title': '无法启动渲染器',
    'err.hint': 'Lumencraft 需要支持浮点渲染目标的 WebGL 2。新版 Chrome、Edge、Firefox 和 Safari 都支持；请确认已开启硬件加速。',
    'err.contextLost': '图形上下文丢失了（显卡被重置或显存不足）。你的世界已保存，刷新页面即可继续。',
    'err.noWebgl2': '这个浏览器不支持 WebGL 2。',
    'err.noFloat': '这块显卡不支持渲染到浮点纹理。',

    'hud.lockHint': '点击画面来转动视角',
    'hud.dragHint': '按住鼠标拖动来转动视角',
    'hud.gameView': '游戏画面',
    'hud.hotbar': '物品栏',
    'toast.newWorld': '新世界已创建',
    'toast.time': '时间 {time}',
    'toast.autoQuality': '当前帧率 {fps}，已把画质切换为"{preset}"。可以在设置里修改。',
    'toast.error': '出错了：{msg}',
    'toast.padConnected': '手柄已连接：{name}',
    'toast.padDisconnected': '手柄已断开',
    'toast.creative': '创造模式',
    'toast.survival': '生存模式',
    'toast.toolBroke': '你的{item}坏了',
    'toast.dusk': '夜幕降临，怪物会在黑暗中出现。',
    'hud.health': '生命值 {n} / {max}',
    'hud.air': '氧气',

    'new.mode': '游戏模式',
    'new.difficulty': '难度',
    'mode.survival': '生存',
    'mode.creative': '创造',
    'mode.survival.desc': '采集、合成，熬过黑夜。天黑以后怪物就会出现。',
    'mode.creative.desc': '所有方块和物品随便用，可以飞行，什么都伤不到你。',
    'diff.peaceful': '和平',
    'diff.easy': '简单',
    'diff.normal': '普通',
    'diff.hard': '困难',
    'set.gameMode': '游戏模式',
    'set.gameMode.desc': '随时在生存和创造之间切换当前世界。',
    'set.difficulty': '难度',
    'set.difficulty.desc': '和平模式没有怪物。难度越高，怪物越多，伤害也越高。',
    'death.title': '你死了',
    'death.note': '你身上的东西都掉在了倒下的地方，趁它们消失之前回去捡。',
    'death.respawn': '重生',
    'death.zombie': '被僵尸杀死了。',
    'death.skeleton': '被骷髅射杀了。',
    'death.arrow': '被箭射死了。',
    'death.spider': '被蜘蛛咬死了。',
    'death.creeper': '被苦力怕炸死了。',
    'death.explosion': '被炸死了。',
    'death.fall': '从高处摔了下来。',
    'death.drown': '淹死了。',
    'death.lava': '试图在熔岩里游泳。',
    'death.fire': '被烧死了。',
    'death.cactus': '被仙人掌扎死了。',
    'death.void': '掉出了这个世界。',
    'death.other': '你的冒险在这里结束了。',

    'title.multi': '多人游戏',
    'mp.title': '和朋友一起玩',
    'mp.hint': '输入 Lumencraft 服务器的地址。同一台服务器上的人共享一个世界，还能语音聊天。',
    'mp.hintHere': '这个页面来自服务器"{server}"，地址留空就能直接加入。',
    'mp.address': '服务器地址',
    'mp.addressPlaceholder': '例如 game.example.com',
    'mp.here': '留空：{server}',
    'mp.name': '你的名字',
    'mp.password': '密码（服务器设置了才需要）',
    'mp.join': '加入',
    'mp.connecting': '正在连接…',
    'mp.err.password': '密码不对。',
    'mp.err.full': '服务器已满。',
    'mp.err.nameTaken': '这个名字已经有人在用了。',
    'mp.err.badName': '先起个名字吧。',
    'mp.err.version': '服务器上的游戏版本和你的不一样，请刷新页面。',
    'mp.err.unreachable': '连不上服务器。请检查地址是否正确、服务器是否在运行。',
    'mp.err.timeout': '服务器没有及时响应。',
    'mp.err.address': '请输入服务器地址。',
    'mp.err.sandbox': '这个预览页面里不能联机，请从你自己的服务器打开游戏。',
    'mp.welcome': '已加入 {server}。按回车聊天，按 V 开关麦克风。',
    'mp.joined': '{name} 加入了游戏',
    'mp.left': '{name} 离开了游戏',
    'mp.died': '{name} 死了',
    'mp.lost': '与服务器的连接断开了',
    'mp.leave': '断开连接并返回标题',
    'mp.players': '{n} 人在线',
    'mp.you': '你',
    'mp.chat': '聊天',
    'mp.chatPlaceholder': '说点什么…回车发送，Esc 关闭',
    'voice.micOff': '开启麦克风',
    'voice.micOn': '麦克风已开启',
    'voice.mute': '麦克风静音',
    'voice.unmute': '取消静音',
    'voice.muted': '麦克风已静音',
    'voice.proximity': '附近语音',
    'voice.global': '全员语音',
    'voice.hint': '附近语音：离得越远声音越小，还能听出对方在哪个方向。全员语音：所有人音量一样。按 V（手柄方向键上）开关麦克风。',
    'voice.speaking': '{names} 正在说话',
    'voice.state.off': '没开麦克风',
    'voice.state.on': '麦克风已开',
    'voice.state.muted': '已静音',
    'voice.state.speaking': '正在说话',
    'voice.err.insecure': '只有通过 https:// 地址打开游戏时才能使用麦克风。',
    'voice.err.denied': '麦克风权限被拒绝了。请在浏览器的网站设置里允许麦克风。',
    'voice.err.unsupported': '这个浏览器不支持语音聊天，但你仍然能听到别人说话。',
    'set.voiceVolume': '语音音量',
    'set.voiceMode': '语音聊天',
    'act.chat': '聊天（多人游戏）',
    'act.mic': '开关麦克风（多人游戏）',
    'touch.chat': '聊天',
    'touch.mic': '麦克风',

    'dev.title': '设备检测',
    'dev.hint': '如果遇到问题，把这个页面截图发给我，并描述一下情况。',
    'dev.browser': '浏览器',
    'dev.screen': '屏幕',
    'dev.gpu': '显卡',
    'dev.webgl2': 'WebGL 2',
    'dev.floatTargets': '浮点渲染目标',
    'dev.maxTexture': '最大纹理尺寸',
    'dev.cores': 'CPU 线程数',
    'dev.memory': '内存',
    'dev.touch': '触摸屏',
    'dev.gamepads': '手柄',
    'dev.gamepadsNone': '没有检测到。连接手柄后随便按一个键。',
    'dev.fps': '帧率',
    'dev.preset': '画质预设',
    'dev.resolution': '渲染分辨率',
    'dev.audio': 'Web Audio 音频',
    'dev.storage': '存档',
    'dev.yes': '支持',
    'dev.no': '不支持',
    'dev.recommend': '建议画质：{preset}',
    'dev.back': '返回',

    'dbg.cpu': 'CPU',
    'dbg.facing': '朝向',
    'dbg.dirs': '北 (-Z)|东 (+X)|南 (+Z)|西 (-X)',
    'dbg.biome': '生物群系',
    'dbg.light': '天空光 {sky} 方块光 {block}',
    'dbg.time': '时间',
    'dbg.weather': '天气',
    'dbg.flying': '飞行中',
    'dbg.swimming': '游泳中',
    'dbg.onGround': '在地面',
    'dbg.airborne': '空中',
    'dbg.chunks': '区块：已加载 {loaded}，绘制 {drawn}，阴影 {shadow}',
    'dbg.render': '渲染 {size}  画质 {preset}  阴影 {shadows}',
    'dbg.off': '关',
    'dbg.seed': '种子',
    'dbg.entities': '实体 {n}   {mode} · {diff}',

    biomes: '海洋|沙滩|平原|森林|白桦林|针叶林|积雪针叶林|沙漠|山地|河流',
  },
};

export const LANGUAGES = ['zh', 'en'];
let current = 'en';
const listeners = new Set();

export function detectLanguage() {
  try {
    const langs = navigator.languages && navigator.languages.length ? navigator.languages : [navigator.language || 'en'];
    for (const l of langs) {
      const s = String(l).toLowerCase();
      if (s.startsWith('zh')) return 'zh';
      if (s.startsWith('en')) return 'en';
    }
  } catch (e) { /* ignore */ }
  return 'en';
}

export function getLanguage() {
  return current;
}

// Adds strings from another module (entities, villagers, multiplayer...).
export function registerStrings(lang, entries) {
  Object.assign(DICT[lang] || (DICT[lang] = {}), entries);
}

export function t(key, vars) {
  let s = DICT[current][key];
  if (s === undefined) s = DICT.en[key];
  if (s === undefined) return key;
  if (vars) s = s.replace(/\{(\w+)\}/g, (m, k) => (vars[k] !== undefined ? String(vars[k]) : m));
  return s;
}

// A list stored as "a|b|c".
export function tList(key) {
  return t(key).split('|');
}

export function blockName(d) {
  if (!d) return '';
  return current === 'zh' && d.zh ? d.zh : d.name;
}

// Items (tools, food, materials) carry the same { name, zh } pair as blocks.
export const itemName = blockName;

export function applyI18n(root = document) {
  for (const el of root.querySelectorAll('[data-i18n]')) el.textContent = t(el.dataset.i18n);
  for (const el of root.querySelectorAll('[data-i18n-html]')) el.innerHTML = t(el.dataset.i18nHtml);
  for (const el of root.querySelectorAll('[data-i18n-attr]')) {
    for (const pair of el.dataset.i18nAttr.split(';')) {
      const [attr, key] = pair.split(':').map((s) => s.trim());
      if (attr && key) el.setAttribute(attr, t(key));
    }
  }
}

export function setLanguage(lang) {
  if (!DICT[lang]) lang = 'en';
  current = lang;
  if (typeof document !== 'undefined') {
    document.documentElement.lang = lang === 'zh' ? 'zh-CN' : 'en';
    applyI18n(document);
  }
  for (const fn of listeners) fn(lang);
}

export function onLanguageChange(fn) {
  listeners.add(fn);
  return () => listeners.delete(fn);
}
